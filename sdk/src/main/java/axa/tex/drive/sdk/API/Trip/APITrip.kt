...
internal class APITrip : KoinComponentCallbacks {
    ...
    constructor(context: Context) {
        ...
        this.uid = DeviceInfo.getUid(context)
    }
    var isRetrievingScoreAutomatically: Boolean = false

    fun sendTrip(tripChunk: TripChunk) {
        LOGGER.info("SENDING PACKET DATA FOR WORKER MANAGER ${tripChunk.data(isRetrievingScoreAutomatically, uid)} tripChunk.isLast: ${tripChunk.isLast} ", function = "sendTrip")
        val fixUploadWork: OneTimeWorkRequest = OneTimeWorkRequest
                ...
                .setInputData(tripChunk.data(isRetrievingScoreAutomatically, uid))
